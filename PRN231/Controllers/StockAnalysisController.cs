...
public class StockAnalysisController : ControllerBase
{
    ...
    public async Task<IActionResult> Predict(string id)
    {
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                /* find probability of increasing and decreasing */
                double countAsc = 0;
                double countDesc = 0;
                for (int i = 0; i < length - 1; i++)
                {
                    if (stocksOverTimeResponses[i + 1].Close - stocksOverTimeResponses[i].Close > 0)
                    {
                        countAsc++;
                    }
                    else
    ...
    /// </summary>
    /// <returns>All stock code in a stock Exchange</returns>
    ///  version: 1
    ///  created by: Nguyễn Thiện Thắng
    ///  created at: 2024/22/2
    ...
    /// <summary>
    /// Map to StockOverTime to StockOverTimeResponse
    /// </summary>
    /// <returns>an array of StockOverTimresponse</returns>
    ///  version: 1
    ///  created by: Nguyễn Thiện Thắng
    ///  created at: 2024/22/2
